...
class GymPursuit(gym.Env):
    ...
    def step(self, action):
        reward = [-0.5] * self.number_of_agents
        ...
        for index_pos, pos in enumerate(self.pos):

            # find prey caught
            if pos in self.prey_pos:
                self.prey_pos.remove(pos)
                reward[index_pos] += 2
                for index_pos_1, pos_1 in enumerate(self.pos):
                    if index_pos_1 != index_pos:
                        if dist_row(pos, pos_1)<self.obs_size/2 and dist_col(pos, pos_1)<self.obs_size/2:
                            reward[index_pos_1] += 1
                        else:
                            reward[index_pos_1] += 0.25
                        

            # find collisions
            if self.pos.count(pos) > 1:
                reward[index_pos] -= 0.5
                new_test_pos = pos
    ...
    def get_state_central(self):
        ...
        for index, pos in enumerate(self.pos):
            this_central = []
            this_central.extend([pos[0] - center_position_of_global_view, pos[1] - center_position_of_global_view])
            for j, pos_j in enumerate(self.pos):
                if j != index:
                    this_central.extend([pos_j[0] - center_position_of_global_view, pos_j[1] - center_position_of_global_view])
                
            for preypos in self.prey_pos:
                this_central.extend([preypos[0] - center_position_of_global_view, preypos[1] - center_position_of_global_view])
            for i in range(len(self.prey_pos), self.number_of_prey):
                this_central.extend([-1, -1]) 
            central_obs.append(this_central)
...
def dist_new(a, b):
    return min(np.abs(a[0] - b[0]),17-np.abs(a[0] - b[0])) + min(np.abs(a[1] - b[1]),17-np.abs(a[1] - b[1]))

def dist_row(a, b):
    return min(np.abs(a[0] - b[0]),17-np.abs(a[0] - b[0]))

def dist_col(a, b):
    return min(np.abs(a[1] - b[1]),17-np.abs(a[1] - b[1]))    
